	...
	public ResponseEntity<?> enviarCorreo(EventoBitacoraDTO eventoBitacoraDTO) {
		...
		try {
			...
			if(eventoBitacoraDTO.perCodigoReasignado() != null){
				 nombreResponsable = eventoBitacoraDTO.perCodigoReasignado().perNombres() + " " + eventoBitacoraDTO.perCodigoReasignado().perApellidos();  //REVISAR
			} else{
...
//						sendEmailUsuario("santiagobermeob@gmail.com", htmlContextBySede);
						break;
					case 3:
